...
def UserLogin(request):
    if request.method == 'POST':
        global usertype
        username = request.POST.get('username', request.POST.get('t1', False))  # Support both old and new parameter names
        password = request.POST.get('password', request.POST.get('t2', False))  # Support both old and new parameter names
        usertype = request.POST.get('role', request.POST.get('t3', False))      # Support both old and new parameter names
        status = 'none'
        for i in range(len(blockchain.chain)):
            if i > 0:
                b = blockchain.chain[i]
                ...
                if arr[0] == "signup":
                    if arr[1] == username and arr[2] == password and arr[6] == usertype:
                        status = 'success'
                        break
        if status == 'success' and (usertype == 'FARMER' or usertype == 'Farmer'):
            file = open('session.txt','w')
            file.write(username)
            file.close()
            context= {'data':"Welcome "+username, 'user': {'username': username, 'role': 'FARMER'}}
            return render(request, 'FarmerScreen.html', context)
        elif status == 'success' and (usertype == 'DRS' or usertype == 'Drs'):
            file = open('session.txt','w')
            file.write(username)
            file.close()
            context= {'data':"Welcome "+username, 'user': {'username': username, 'role': 'DRS'}}
            return render(request, 'DRSScreen.html', context)
        elif status == 'success' and (usertype == 'MILLER' or usertype == 'Miller'):
            file = open('session.txt','w')
            file.write(username)
            file.close()
            context= {'data':"Welcome "+username, 'user': {'username': username, 'role': 'MILLER'}}
            return render(request, 'MillerScreen.html', context)
        else:
...
def FarmerScreen(request):
    if request.method == 'GET':
        ...
        context = {'user': {'username': username, 'role': 'FARMER'}}
        return render(request, 'FarmerScreen.html', context)

def MillerScreen(request):
    if request.method == 'GET':
        username = ""
        if os.path.exists('session.txt'):
            file = open('session.txt', 'r')
        ...
        context = {'user': {'username': username, 'role': 'MILLER'}}
        return render(request, 'MillerScreen.html', context)

def DRSScreen(request):
    if request.method == 'GET':
        username = ""
        if os.path.exists('session.txt'):
            file = open('session.txt', 'r')
        ...
        context = {'user': {'username': username, 'role': 'DRS'}}
        return render(request, 'DRSScreen.html', context)
